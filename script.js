        ...
        const ICONS = {
            ...
            bug: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v4M8 4l2 4M16 4l-2 4M20 12h-3M4 12H1M8 20H5M16 20h3M12 20v-4M4 12v4M20 12v4M16 16H8M12 16v-4"/></svg>`,
            lightbulb: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18h6M10 21h4M12 2C8 2 5 5 5 9c0 2.5 1.5 4.5 3 6h8c1.5-1.5 3-3.5 3-6 0-4-3-7-7-7z"/><path d="M12 12v2"/></svg>`,
        ...
        async function initializeSecureWorkspace(payload, token) {
            ...
            // FIX: Admin button controlled by loadUserProfile, not hardcoded email.
    ...
    document.getElementById('admin-dashboard-btn').style.display = 'block';
        ...
        async function executeCommand(isRetry = false) {
            ...
            // Remove "Tech Beast Mode" – just send the command as-is.
            // The system prompt already contains the conciseness directive.
    ...
    const auth = document.getElementById('g_id_onload');
    // Force re-prompt by calling google.accounts.id.prompt()
...
function toggleTheme() {
    ...
    localStorage.setItem('axelr_theme', document.body.classList.contains('dark-theme') ? 'dark' : 'light');
}

// On load, apply saved theme
(function() {
    const theme = localStorage.getItem('axelr_theme');
    if (theme === 'dark') document.body.classList.add('dark-theme');
})();

// Add another account – trigger Google sign-in again
function addAnotherAccount() {
    // Google's GSISDK provides a sign-in method
    // We can programmatically trigger the sign-in button
    const signinBtn = document.querySelector('.g_id_signin');
    if (signinBtn) signinBtn.click();
    else {
        // Fallback: reload with prompt
        const clientId = '474929925590-a0it7ijp845oqbni72iaqpsvqdvnu0jd.apps.googleusercontent.com';
        const auth2 = gapi.auth2.getAuthInstance();
        if (auth2) auth2.signIn().then(handleCredentialResponse);
    }
}
...
function openPrivacyModal() {
    closeModals();
    // You can create a new modal or use an existing one; for simplicity, we'll show an alert with options
    alert("Privacy & Security settings:\n- Two-factor authentication (coming soon)\n- Session management (coming soon)\n- Data export (coming soon)");
}

// Help Center modal (FAQs)
function openHelpModal() {
    closeModals();
    // Show a dedicated modal with FAQs; we'll reuse a modal or show an alert with links
    alert("Help Center:\nFAQs:\n1. How to use Axelr? - See our docs at https://docs.axelr.in\n2. Billing questions? - Contact support\n3. Report a bug - Use the feedback option");
}

// Updates modal
function openUpdatesModal() {
    closeModals();
    alert("Updates & Notifications:\n- You are on the latest version (v3.1).\n- Enable release notifications: (toggle coming soon)");
}
...
function openPrivacy() {
    // Open a new modal with privacy settings
    closeModals();
    ...
    modal.innerHTML = `
        <div class="modal-card">
            ...
            <div style="padding:10px 0;">
                ...
                <hr>
                <h4>Cookies</h4>
                <p>We use essential cookies only. Manage in your browser settings.</p>
            </div>
        </div>
    `;
    document.body.appendChild(modal);
    // override closeModals to remove it
    const origClose = closeModals;
    closeModals = function() {
        document.querySelectorAll('.modal-overlay').forEach(m => m.remove());
        closeModals = origClose;
    };
}
...
function openHelpCenter() {
    ...
    modal.innerHTML = `
        <div class="modal-card">
            ...
            <div style="padding:10px 0;">
                <details><summary>How do I upload files?</summary><p>Use the attach button next to the input box.</p></details>
                <details><summary>What is the free tier limit?</summary><p>5 extractions/generations per day.</p></details>
                <details><summary>How to upgrade?</summary><p>Go to Settings → Workspace Plan.</p></details>
                <details><summary>Contact Support</summary><p>Email: support@axelr.in</p></details>
            </div>
        </div>
    `;
    document.body.appendChild(modal);
    // override closeModals similarly
}
        // ============================================================
        // INIT
        // ============================================================
